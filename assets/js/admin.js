(function ($) {
  ...
  if (
    $("#post_type").val() === "review-rating" ||
    $("#post_type").val() == "enquiries"
  ) {
  ...
  jQuery(".turio_tour_action").click(function (event) {
    ...
    var data = {
      action: "saasty_tour_rating_action",
      post_id: post_id,
    ...
    $.ajax({
      ...
      success: function () {
        if (post_status == "approve") {
          jQuery('.turio_tour_action[post-id="' + post_id + '"]')
            .empty()
            .html("Unapprove");
          ...
          jQuery('.turio_tour_action[post-id="' + post_id + '"]')
            .empty()
            .html("Approve");
          jQuery('.turio_tour_action[post-id="' + post_id + '"]').val(
            "approve"
          );
        }
      },
