...
<?php if (have_comments() || comments_open()) : ?>
    <div id="comments" class="postbox-comment mb-80">
        ...
        <?php if (get_comment_pages_count() > 1 && get_option('page_comments')): ?>
            <div class="comment-pagination mb-50 d-none">
                <nav id="comment-nav-below" class="comment-navigation" role="navigation">
                    <h1 class="screen-reader-text"><?php esc_html_e('Comment navigation', 'saasty'); ?></h1>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="nav-previous "><?php previous_comments_link(esc_html__('&larr; Older ', 'saasty')); ?></div>
                        </div>
                        <div class="col-md-6">
                            <div class="nav-next "><?php next_comments_link(esc_html__('Newer &rarr;', 'saasty')); ?></div>
                        </div>
                    </div>
        ...
        ',
            ...
            'must_log_in'        => '
            <p class="must-log-in">
            ' . esc_html__('You must be', 'saasty') . ' <a href="' . esc_url(wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '">' . esc_html__('logged in', 'saasty') . '</a> ' . esc_html__('to post a comment.', 'saasty') . '
            </p>',
            /** This filter is documented in wp-includes/link-template.php */
            'logged_in_as'       => '
            <p class="logged-in-as">
            ' . esc_html__('Logged in as', 'saasty') . ' <a href="' . esc_url(get_edit_user_link()) . '">' . esc_html($user_identity) . '</a>. <a href="' . esc_url(wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '" title="' . esc_attr__('Log out of this account', 'saasty') . '">' . esc_html__('Log out?', 'saasty') . '</a>
            </p>',
            'id_form'            => 'commentform',
            'id_submit'          => 'submit',
            'class_submit'       => 'tp-btn',
            'title_reply'        => esc_html__('Leave A Comment', 'saasty'),
            'title_reply_to'     => esc_html__('Leave a Reply to %s', 'saasty'),
            'cancel_reply_link'  => esc_html__('Cancel reply', 'saasty'),
            'label_submit'       => esc_html__('Post Comment', 'saasty'),
            'format'             => 'xhtml',
        ];
